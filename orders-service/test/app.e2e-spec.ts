...
describe('Orders Service (e2e)', () => {
  ...
  describe('POST /orders', () => {
    it('crée une commande et retourne 201', async () => {
      const res = await request(app.getHttpServer())
        .post('/orders')
        .send({ userId: 'user-1', items: [validItem] });
  ...
  describe('GET /orders/:id', () => {
    it('retourne la commande par son id', async () => {
      const created = await request(app.getHttpServer())
        .post('/orders')
        .send({ userId: 'user-2', items: [validItem] });
  ...
  describe('PATCH /orders/:id/status', () => {
    it('passe le statut de CREATED à PAID', async () => {
      const created = await request(app.getHttpServer())
        .post('/orders')
        .send({ userId: 'user-3', items: [validItem] });
    ...
    it('passe le statut de PAID à SHIPPED', async () => {
      const created = await request(app.getHttpServer())
        .post('/orders')
        .send({ userId: 'user-4', items: [validItem] });
    ...
    it('retourne 400 si la transition CREATED → SHIPPED est invalide', async () => {
      const created = await request(app.getHttpServer())
        .post('/orders')
        .send({ userId: 'user-5', items: [validItem] });
    ...
    it('retourne 400 si le statut est invalide', async () => {
      const created = await request(app.getHttpServer())
        .post('/orders')
        .send({ userId: 'user-6', items: [validItem] });
